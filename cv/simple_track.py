...
out = cv2.VideoWriter(
    "output3_tracking_only_ball.mp4", cv2.VideoWriter_fourcc(*"mp4v"), fps, (width, height)
)
...
parser.add_argument("--corners", help="JSON file with table corners", default="table_corners4.json")
...
while cap.isOpened():
    ...
    cv2.putText(
        top_view, "Zone 1", (MID_X + 100, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2
    )
    cv2.putText(
        top_view, "Zone 4", (100, MID_Y + 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2
    )
    cv2.putText(
        top_view,
        "Zone 2",
    ...
    if results.boxes is not None and len(results.boxes) > 0:
        ...
        for box, cls, conf in zip(boxes, classes, confidences):
            ...
            if cls == 0:  # только мяч — рисуем траекторию (простой "трейл" без ID)
                ...
                loser = rally.step(
                    event, side
                )  # возвращает проигравшего розыгрыша, если розыгрыш закончился
                ...
                if loser:
                    ...
                    if current_game.finished:
                        ...
                        print(
                            f"Game finished! Winner: {'LEFT' if game_winner == LEFT else 'RIGHT'}"
                        )
                        ...
                        if match.games_won[game_winner] >= match.games_to_win:
                            ...
                            print(
                                f"MATCH FINISHED → Winner: {'LEFT' if match.winner == LEFT else 'RIGHT'}"
                            )
                        else:
                ...
                if len(pts_tv) > 1:
                    cv2.polylines(
                        top_view, [pts_tv], isClosed=False, color=(0, 255, 0), thickness=3
                    )
                zone = get_zone(mx, my)
                cv2.putText(
                    top_view,
